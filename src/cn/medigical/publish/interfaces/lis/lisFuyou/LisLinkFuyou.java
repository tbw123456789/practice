package cn.medigical.publish.interfaces.lis.lisFuyou;
...
public class LisLinkFuyou {
    private static LisLinkFuyou instance;
    protected Logger logger = Logger.getLogger("LisLinkFuyou");

    public static LisLinkFuyou getInstance() {
        if (instance == null) {
            instance = new LisLinkFuyou();
        }
        return instance;
    }
    ...
    public PatientLisData GetPatientLabTestMaster(String id) {
        clear();
        PatientLisData patientLisData = null;
        ...
        if (patientLink != null) {
            ...
            if (itemTypeList.size() > 0) {
                ...
                patientLisData = patientLisDataDao.getPatientLisData8PatientId(id);
                    ...
                    for (cn.medigical.publish.pojo.lis.jianyanjgxm.ITEMType itemType : itemTypesLX) {
                        ...
                        if (itemType.getReportItemName().contains("免疫全套")) {
                            if (itemType.getReportParmNameSX().equalsIgnoreCase("HBsAg")) {
                                ...
                                if (StringUtil.ToDouble(itemType.getResult()) >= HBSAG||itemType.getResult().equals("> 250.00")) {
                                    HBsAg = "(+)";
                                } else {
                                    HBsAg = "(-)";
                                }

...
public class ClientPLZYYListenerServiceImpl implements ClientListenerService{
	...
	public boolean clientInteraction(String xmlString) {
		...
		if (ServicesSign.OPERATION_ADD.equals(udpMsgAnalysis.getMethodId())) {
			Boolean flag = HisLinkPLZYY.getInstance().getOperationFromHis();			
		}
...
//		}
		...
		if (ServicesSign.ACCCOUNT_ADD.equals(udpMsgAnalysis.getMethodId())) {
			List<Account> account = HisLinkPLZYY.getInstance().getAccountFromHis();			
			UdpHelper.sendMsg("successed");
		}

		if (ServicesSign.DEPART_ADD.equals(udpMsgAnalysis.getMethodId())) {
			List<ConfigDepartments> configDepartments = HisLinkPLZYY.getInstance().getDepartmentFromHis();

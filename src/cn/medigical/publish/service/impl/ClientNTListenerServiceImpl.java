...
public class ClientNTListenerServiceImpl implements ClientListenerService {
    ...
    public boolean clientInteraction(String xmlString) {
        ...
        if (ServicesSign.OPERATION_ADD.equals(udpMsgAnalysis.getMethodId())) {
            HisLinkNantong.getInstance().getOperationFromHis();
            UdpHelper.sendMsg("successed");
        }
        if (ServicesSign.PATIENT_ADD.equals(udpMsgAnalysis.getMethodId())) {
            HisLinkNantong.getInstance().getBasicPatInfoOperation(udpModel.getZyh());
            UdpHelper.sendMsg("successed");
        }

        if (ServicesSign.ACCCOUNT_ADD.equals(udpMsgAnalysis.getMethodId())) {
            HisLinkNantong.getInstance() .getAccountFromHis();
            UdpHelper.sendMsg("successed");
        }

        if (ServicesSign.DEPART_ADD.equals(udpMsgAnalysis.getMethodId())) {
            HisLinkNantong .getInstance().getDepartmentFromHis();
            UdpHelper.sendMsg("successed");
        }

        if (ServicesSign.GETLISDATA.equals(udpMsgAnalysis.getMethodId())) {
            LisDataNantong2.getInstance().getLisFromHis(udpModel.getPatientId(), udpModel.getZyh(),udpModel.getPatId(),udpModel.getZycs());
        }
        return false;
    }
}
